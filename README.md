...
### 🧨 保护机制
- **实体爆炸保护** - 防止TNT、苦力怕等实体爆炸破坏熔炉
- **方块爆炸保护** - 防止TNT方块等方块爆炸破坏熔炉
- **可配置保护** - 每种熔炉类型可单独配置是否启用保护

### ⚙️ 配置方式
```yaml
furnaces:
  furnace:
    explosion-protection: true   # 启用爆炸保护
  blast_furnace:
    explosion-protection: false  # 禁用爆炸保护
  smoker:
    explosion-protection: true   # 启用爆炸保护
```

### 🔍 工作原理
1. **事件监听** - 监听EntityExplodeEvent和BlockExplodeEvent
2. **熔炉检测** - 检查爆炸影响范围内的自定义熔炉
3. **保护判断** - 根据配置决定是否保护该类型熔炉
4. **移除保护** - 从爆炸列表中移除受保护的熔炉方块
5. **调试记录** - 记录保护操作的详细信息
...
<<<<<<< HEAD
    # 爆炸保护设置
    explosion-protection: true  # 是否保护熔炉免受爆炸破坏
=======
>>>>>>> 4569721dc3ab24fb6fb025bd2101694763174417
    vault:  # 金币升级
      max-level: 3
      speed:
        1: 200
  ...
  blast_furnace:  # 高炉配置
    explosion-protection: true  # 爆炸保护
    # ... 类似结构
